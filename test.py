...
def main():
    ...
    try:
        ...
        plt.plot(meta_losses, label="Meta-Training Loss", color='blue', alpha=0.3)
        smoothed = np.convolve(meta_losses, np.ones(20)/20, mode='valid')
        plt.plot(smoothed, color='darkblue', linewidth=2, label="Smoothed Loss")
    ...
    plt.plot(steps, maml_nmse_history, marker='o', color='green', linewidth=2, label="MAML (Ours)")
    plt.plot(steps, base_nmse_history, marker='s', color='red', linewidth=2, linestyle='--', label="Baseline")
